...
// СП1 — страховая часть за стаж ДО введения персонифицированного учёта (01.01.1996).
// Закон №57, ст.22 п.3«а» (сверено вербатим 2026-08-02): СП1 = С × З, где
//   С — 1% за каждый полный год стажа до персучёта; при назначении по возрасту
//       засчитывается не более 17 лет мужчинам / 12 лет женщинам (кап ежегодно снижается);
//   З — заработок для исчисления: 20 расчётных показателей × 200% (стаж до 15 лет)
//       или × 250% (15–20 лет) = 4 000 / 5 000 сом. НЕ зарплата пользователя.
const RP = 100; // расчётный показатель, сом
const SP1_BASE_20RP = 20 * RP; // 2 000 сом
const SP1_CAP_YEARS_MALE = 17;
const SP1_CAP_YEARS_FEMALE = 12;
...
const PensionCalculatorPage = () => {
  ...
  const calculatePension = () => {
    ...
    // Страховая часть 1 (за стаж до персучёта, ст.22 п.3«а»): СП1 = С × З.
    // Зарплата пользователя в СП1 НЕ участвует — З фиксирован от 20 РП.
    const capYears = gender === 'male' ? SP1_CAP_YEARS_MALE : SP1_CAP_YEARS_FEMALE;
    const yearsBefore1996 = Math.min(Math.floor(experienceBefore1996.months / 12), capYears);
    const sp1Zarabotok = SP1_BASE_20RP * (yearsBefore1996 >= 15 ? 2.5 : 2.0); // 4000 / 5000 сом
    const insurancePart1 = yearsBefore1996 > 0 ? (yearsBefore1996 * 0.01) * sp1Zarabotok : 0;
              ...
              <div className="border border-gray-200 rounded-lg p-5">
                ...
                <div className="bg-gray-50 rounded p-4">
                  ...
                  <p className="text-sm text-gray-600 mt-2">
                    {language === 'ky'
                      ? 'Дайындалган пенсиялардын жалпы өлчөмү 2025-жылдын 1-октябрынан тартып 7 100 сомдон кем боло албайт (Министрлер Кабинетинин 28.08.2025-ж. №534 токтому). 2026-жылдын башына орточо пенсия — 11 238 сом (Соцфонддун расмий жыйынтыктары).'
                      : 'Общий размер назначаемых пенсий с 1 октября 2025 года не может быть меньше 7 100 сом (постановление Кабмина №534 от 28.08.2025). Средняя пенсия на начало 2026 года — 11 238 сом (официальные итоги Соцфонда).'}
                  </p>
                </div>
              </div>
